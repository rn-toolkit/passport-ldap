...
var handleAuthentication = function(req, options) {
  ...
  var userdap2 = new Ldap.createClient({
    url: 'ldap://192.168.100.3:389'
  });
  userdap2.on('connectError', function(errr) {
    console.log('userdap2 connectError', errr);
  });
  userdap2.on('connect', function() {
    console.log('connected');
  });

  userdap2.bind('liangwenwei@ut.cn', '1129#LWW', function(err, user) {
    console.log('userdap2', err, user);
  });


  ldap = new Ldap.createClient({
    url: 'ldap://192.168.100.3:389'
  });
  ldap.on('connectError', errorHandler);
  ldap.on('connect', function() {
    console.log('connected');
  });
  console.log('ldap', username, password, this.options.server);

  ldap.bind('liangwenwei@ut.cn', '1129#LWW', function(err, user) {
    // ldap.unbind();
